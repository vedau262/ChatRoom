package com.festfive.app.customize.view;
...
public class ShapeRoundView extends ShapeBaseView {
    ...
    public ShapeRoundView(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public ShapeRoundView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public ShapeRoundView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.ShapeRoundView);
            borderColor = attributes.getColor(R.styleable.ShapeRoundView_shape_round_border_color, borderColor);
            borderWidthPx = attributes.getDimensionPixelSize(R.styleable.ShapeRoundView_shape_round_border_width, borderWidthPx);
            shapeRadius = attributes.getDimensionPixelSize(R.styleable.ShapeRoundView_shape_round_radius, shapeRadius);
            attributes.recycle();
        }
        ...
        super.setClipPathCreator(new ClipPathManager.ClipPathCreator() {
            ...
            public Path createClipPath(int width, int height) {
                ...
                return generatePath(rectF,
                        limitSize(shapeRadius, width, height),
                        limitSize(shapeRadius, width, height),
                        limitSize(shapeRadius, width, height),
                        limitSize(shapeRadius, width, height)
                );
